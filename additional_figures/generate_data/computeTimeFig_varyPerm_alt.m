...
for rrr=1:ntrep
	for Nperm=nvals
		...
		fprintf('  timing 1\n')
		[mm1,~] = repTimeFunc(@evalFtestPower,1,tmeth,tt,freq,acro,Amp);

		fprintf('  timing 2\n')
		[mm2,~] = repTimeFunc(wrap_Tinf,1,tmeth,tt,freq,acro,Amp);

		fprintf('  timing 3\n')
		[mm3,~] = repTimeFunc(wrap_T2,1,tmeth,tt,freq,acro,Amp);

		fprintf('  timing 4\n')
		[mm4,~] = repTimeFunc(wrap_T2b,1,tmeth,tt,freq,acro,Amp);
		datloc =   [Nperm mm1 NaN NaN 1;
					Nperm mm2 NaN NaN 2;
					Nperm mm3 NaN NaN 3;
					Nperm mm4 NaN NaN 4];
		dat = [dat;datloc];
...
writematrix(dat,sprintf('../data/compTimes_varyPerm_method%s_alt.csv',tmeth));
