...
pdf = c(1,6) |> lapply(function(freq){
  ...
  df_irr  = data.frame(meas='alternative',acro=acrovec,pval= matrixTests::row_cosinor(Yirr,tirr,1/freq) %>% {.$pvalue}) 
  ...
  df$meas = factor(df$meas,levels=c('true','equispaced','alternative'))
  df$freq=freq
  return(df)
...
cmap_cust = c('true1'=rgb(.05,0.5,.06),
          'alternative1'=rgb(.05,0.5,.06),
          'equispaced1'=rgb(.05,0.5,.06),
          'true6'=rgb(.05,0.5,.06),
          'alternative6'=rgb(.05,0.5,.06),
...
df=rbind(data.frame(time=tunif,type='equispaced'),
         data.frame(time=tirr,type='alternative'))
dat_bands = data.frame(start=c(0:12)*2,end=(c(0:12)*2+1))
head(df)
df$type = factor(df$type,levels=c('equispaced','alternative'))
